...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchJson = async (url: string) => {
      ...
      const res = await fetch(fullUrl, { headers, signal: ac.signal, cache: "no-store" });
      ...
      if (contentType.includes("text/html") || text.trim().startsWith("<")) {
        throw new Error(
          `Expected JSON from ${fullUrl} but received HTML/other. Response snippet:\n\n${text.slice(
            0,
            800
          )}`
        );
      ...
      try {
        ...
        throw new Error(
          `Invalid JSON from ${fullUrl}: ${(e as Error).message
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <div
            key={stat.title}
            className={`bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 ${isVisible
          ...
          >
            ...
            {stat.total !== undefined && (
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all duration-500 ${stat.color.includes("blue")
                    ? "bg-blue-500"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center">
              <Calendar className="w-5 h-5 text-green-600 mr-2" /> Upcoming
              Tasks
            </h3>
